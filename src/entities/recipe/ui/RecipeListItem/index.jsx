...
const RecipeListItem = ({ recipe }) => {
  ...
  return (
    <Link to={RouterPath.recipe_detail.replace(":id", recipe.id)} className="block overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <article className="grid gap-4 md:grid-cols-[200px_minmax(0,1fr)_auto]">
        {recipe?.image ? <img src={recipe.image} alt={recipe.name ?? "Recipe"} className="h-full min-h-44 w-full bg-slate-100 object-cover" /> : <div className="flex h-44 items-center justify-center text-sm text-slate-400">No image</div>}

        <div className="flex min-w-0 flex-col justify-between gap-4 py-4">
          <div className="space-y-2">
            <h3 className="text-2xl font-semibold text-slate-900">{recipe?.name ?? "RecipeListItem"}</h3>
            {/* Описание */}
            <p className="max-w-2xl text-sm text-slate-500">{recipe?.description ?? `A simple and delicious ${recipe?.cuisine?.toLowerCase() || "cuisine"} classic with fresh ingredients.`}</p>
          </div>
